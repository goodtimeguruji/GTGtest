...
//
// Accepts a single address, a comma/semicolon-separated string of several
// addresses, or an array of addresses — used for both `to` and `cc` so the
// caller can send to more than one person and CC others in the same request.
...
function parseEmailList(input) {
  ...
  for (const item of raw) {
    ...
    if (!addr) continue;
    if (!EMAIL_RE.test(addr)) continue; // silently skip malformed entries
...
export const sendMuhuratEmail = async (req, res) => {
  try {
    const { filtered = [], common = [], selectedFunction = "Muhurat", to, cc } = req.body;
    ...
    // `to` defaults to the logged-in user's own email (unchanged existing
    // behaviour) but can be overridden with one or more explicit addresses —
    // e.g. { to: "a@x.com, b@x.com", cc: "c@x.com" } — to send to whomever
    // is needed, directly from this endpoint. `cc` is optional and additive.
    const toList = to ? parseEmailList(to) : [loggedInEmail];
