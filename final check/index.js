...
app.use(cors({ origin: ALLOWED_ORIGIN, methods: ["GET","POST"], allowedHeaders: ["Content-Type","Authorization"], credentials: true }));
...
const setupRoute = (routePath, handler) => {
  app.post(routePath, muhuratLimiter, verifyToken, async (req, res, next) => {
    try {
      ...
      const result = await handler(validated.fromDateStr, validated.toDateStr, validated.userNakshatra, validated.userRasi, validated.lat, validated.lon, validated.tzone, validated.place);
      res.json(result);
